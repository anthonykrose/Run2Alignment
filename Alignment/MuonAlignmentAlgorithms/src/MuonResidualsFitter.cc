...
{
//  std::cout<<"MuonResidualsFitter::selectPeakResiduals nsigma = "<<nsigma<<" nvar = "<<nvar << std::endl;
//  std::cout << "                                       numResiduals(): " << numResiduals() << std::endl;
...
//  std::cout << "+++++ JUST after N residuals" << std::endl;
  //just to be sure (can't see why it might ever be more then 10)
  assert(nvar<=10 && nvar>0);

  std::vector<double*>::iterator r = m_residuals.begin();
//  std::cout << "+++++ JUST before if (nvar==1)" << std::endl;
...
//  std::cout << "+++++ JUST after if (nvar==1)" << std::endl;
  ...
  while (r != m_residuals.end())
  {
//    std::cout << "+++++ Inside loop while (r != m_residuals.end())" << std::endl;
    double *row = new double[nvar];
    for (int v = 0; v<nvar; v++)  row[v] = (*r)[ vars[v] ];
    std::cout << "counter1 " << counter1 << "row[0] " << row[0] << " row[1] " << row[1] << std::endl;
    re.AddRow(row);
//    delete[] row;
    ++r;
    counter1++;
  ...
  Cov.Invert();
  std::cout << "+++++ JUST after Cov.Invert();" << std::endl;
...
void MuonResidualsFitter::fiducialCuts(unsigned int idx) {
  ...
  double yMin = -80.0;
  double yMax =  80.0;
  double phiMin = -0.07; // 10 deg = 0.1745 rad
  double phiMax =  0.07;
  ...
  if ( id.subdetId() == MuonSubdetId::DT ) {
    ...
    // isDT      = true;
    dtWheel   = chamberId.wheel();
    dtStation = chamberId.station();
    dtSector  = chamberId.sector();
    if ( lDebug) std::cout << "This is DT wheel " << dtWheel << " station " << dtStation << " sector " << dtSector << std::endl;
  } else if ( id.subdetId() == MuonSubdetId::CSC ) {
    CSCDetId chamberId(id.rawId());
    // isCSC      = true;
    cscEndcap  = chamberId.endcap();
    cscStation = chamberId.station();
    cscRing    = chamberId.ring();
    cscChamber = chamberId.chamber();
    if ( lDebug) std::cout << "This is CSC endcap " << cscEndcap << " station " << cscStation << " ring " << cscRing << " chamber " << cscChamber << std::endl;
  } else {
    if ( lDebug) std::cout << "Warning! Unknown MuonSubdetId" << std::endl;
  }
  ...
  for (std::vector<double*>::const_iterator r = residuals_begin();  r != residuals_end();  ++r) {
    ...
    if ( type() == MuonResidualsFitter::k6DOF ) {
      if ( lDebug) std::cout << " This is MuonResidualsFitter::k6DOF" << std::endl;
      double positionX = (*r)[4];
      if (positionX >= xMin && positionX <= xMax) {
        if ( lDebug) std::cout << "   Track within fiducial cut: xMin = " << xMin << " x = " << positionX << " xMax = " << xMax << std::endl;
      } else {
        if ( lDebug) std::cout << "   Track outside fiducial cut: xMin = " << xMin << " x = " << positionX << " xMax = " << xMax << std::endl;
        m_residuals_ok[iResidual] = false;
      }
      double positionY = (*r)[5];
      if (positionY >= yMin && positionY <= yMax) {
        if ( lDebug) std::cout << "   Track within fiducial cut: yMin = " << yMin << " y = " << positionY << " yMax = " << yMax << std::endl;
      } else {
        if ( lDebug) std::cout << "   Track outside fiducial cut: yMin = " << yMin << " y = " << positionY << " yMax = " << yMax << std::endl;
        m_residuals_ok[iResidual] = false;
      }
    } else if ( type() == MuonResidualsFitter::k5DOF ) {
      if ( lDebug) std::cout << " This is MuonResidualsFitter::k5DOF" << std::endl;
      double positionX = (*r)[2];
      if (positionX >= xMin && positionX <= xMax) {
        if ( lDebug) std::cout << "   Track within fiducial cut: xMin = " << xMin << " x = " << positionX << " xMax = " << xMax << std::endl;
      } else {
        if ( lDebug) std::cout << "   Track outside fiducial cut: xMin = " << xMin << " x = " << positionX << " xMax = " << xMax << std::endl;
    ...
    } else if ( type() == MuonResidualsFitter::k6DOFrphi ) {
      if ( lDebug) std::cout << " This is MuonResidualsFitter::k6DOFrphi" << std::endl;
      double positionX = (*r)[2];
      double positionY = (*r)[3];
      if ( lDebug) std::cout << "   Track x " << positionX << " y " << positionY << std::endl;
      double R = 500.0;
      double cscDphi = 1; // 1 - 10 deg chamber; 2 - 20 deg chamber
      if ( cscStation == 1 && cscRing == 1 ) {
        R = 181.5;
        cscDphi = 1.0;
      } else if ( cscStation == 1 && cscRing == 2 ) {
        R = 369.7;
        cscDphi = 1.0;
      } else if ( cscStation == 1 && cscRing == 3 ) {
        R = 595.15;
        cscDphi = 1.0;
      } else if ( cscStation == 2 && cscRing == 1 ) {
        R = 242.7;
        cscDphi = 2.0;
      } else if ( cscStation == 3 && cscRing == 1 ) {
        R = 252.7;
        cscDphi = 2.0;
      } else if ( cscStation == 4 && cscRing == 1 ) {
        R = 262.65;
        cscDphi = 2.0;
      } else if ( ( cscStation == 2 || cscStation == 3 || cscStation == 4 ) && cscRing == 2 ) {
        R = 526.5;
        cscDphi = 1.0;
      } else {
        if ( lDebug) std::cout << "Warning! Unknown CSC station " << cscStation << " ring " << cscRing << std::endl;
      }
      if ( lDebug) std::cout << "   Chamber R " << R << std::endl;
      double phi = atan( positionX / (R + positionY) );
      if ( phi >= cscDphi*phiMin && phi <= cscDphi*phiMax ) {
        if ( lDebug) std::cout << "   Track within fiducial cut: phiMin = " << phiMin << " phi = " << phi << " phiMax = " << phiMax << std::endl;
      } else {
        if ( lDebug) std::cout << "   Track outside fiducial cut: phiMin = " << phiMin << " phi = " << phi << " phiMax = " << phiMax << std::endl;
        m_residuals_ok[iResidual] = false;
      }
    } else {
