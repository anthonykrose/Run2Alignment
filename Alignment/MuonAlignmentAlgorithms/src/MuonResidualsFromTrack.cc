...
{
  bool verbose = false;
  if(verbose) std::cout << "BEGIN MuonResidualsFromTrack" << std::endl;
  ...
  for (trackingRecHit_iterator hit = m_recoTrack->recHitsBegin(); hit != m_recoTrack->recHitsEnd(); ++hit) {
    if((*hit)->isValid()) {
      ...
      if ( hitId.det() == DetId::Tracker ) {
        iT++;
          if(verbose) std::cout << "Tracker Hit " << iT << " is found. Add to refit. Dimension: " << (*hit)->dimension() << std::endl;
...
//        }
          iM++;
	  if (verbose) std::cout << "Muon Hit " << iM << " is found. We do not add muon hits to refit. Dimension: " << (*hit)->dimension() << std::endl;
          if ( hitId.subdetId() == MuonSubdetId::DT ) {
            const DTChamberId chamberId(hitId.rawId());
            if (verbose) std::cout << "Muon Hit in DT wheel " << chamberId.wheel() << " station " << chamberId.station() << " sector " << chamberId.sector() << "." << std::endl;
          } else if ( hitId.subdetId() == MuonSubdetId::CSC ) {
            const CSCDetId cscDetId(hitId.rawId());
            if (verbose) std::cout << "Muon hit in CSC endcap " << cscDetId.endcap() << " station " << cscDetId.station() << " ring " << cscDetId.ring() << " chamber " << cscDetId.chamber() << "." << std::endl;
          } else if ( hitId.subdetId() == MuonSubdetId::RPC ) {
            if (verbose) std::cout << "Muon Hit in RPC" << std::endl;
          } else {
            if (verbose) std::cout << "Warning! Muon Hit not in DT or CSC or RPC" << std::endl;
          }
...
//  std::cout << "Tracker trajectories size " << vTrackerTrajectory.size() << std::endl;
  ...
  if (verbose) std::cout << "  Size of vector of TrajectoryMeasurements: " << vTrajMeasurement.size() << std::endl;
  ...
  for ( std::vector<TrajectoryMeasurement>::const_iterator iTrajMeasurement =  vTrajMeasurement.begin();
                                                         ...
                                                         ++iTrajMeasurement ) {
    nTrajMeasurement++;
    if (verbose) std::cout << "    TrajectoryMeasurement #" << nTrajMeasurement << std::endl;
    ...
    if (verbose) std::cout << "      TrajectoryMeasurement TSOS validity: " << tsos.isValid() << std::endl;
    if ( tsos.isValid() ) {
      double tsosGlobalPositionR = sqrt( tsos.globalPosition().x()*tsos.globalPosition().x() + tsos.globalPosition().y()*tsos.globalPosition().y() );
      if (verbose) std::cout << "         TrajectoryMeasurement TSOS localPosition"
                << " x: " << tsos.localPosition().x()
                << " y: " << tsos.localPosition().y()
                << " z: " << tsos.localPosition().z()
                << std::endl;
      if (verbose) std::cout << "         TrajectoryMeasurement TSOS globalPosition"
    ...
    if (verbose) std::cout << "      TrajectoryMeasurement hit validity: " << trajMeasurementHit->isValid() << std::endl;
    if ( trajMeasurementHit->isValid() ) {
      ...
      if ( trajMeasurementHitId.det() == DetId::Tracker ) {
        if (verbose) std::cout << "      TrajectoryMeasurement hit Det: Tracker" << std::endl;
        if (verbose) std::cout << "      TrajectoryMeasurement hit dimension: " << trajMeasurementHitDim << std::endl;
        m_tracker_numHits++;
...
// YP I add false here. No trajectory measurments in Muon system if we corrected TrackTransformer accordingly
        ...
        if (verbose) std::cout << "      TrajectoryMeasurement hit Det: Muon" << std::endl;
        
        if ( trajMeasurementHitId.subdetId() == MuonSubdetId::DT ) {
          const DTChamberId chamberId(trajMeasurementHitId.rawId());
          if (verbose) std::cout << "        TrajectoryMeasurement hit subDet: DT wheel " << chamberId.wheel() << " station " << chamberId.station() << " sector " << chamberId.sector() << std::endl;
...
//          if (verbose) std::cout << "           The chamber position in global frame x: " << gChX << " y: " << gChY << " z: " << gChZ << std::endl;
	  ...
	  if (verbose) std::cout << "AR hitX: " << hitX << std::endl;
	  if (verbose) std::cout << "AR hitY: " << hitY << std::endl;
	  if (verbose) std::cout << "AR hitZ: " << hitZ << std::endl;

          double tsosX = tsos.localPosition().x();
          double tsosY = tsos.localPosition().y();
          double tsosZ = tsos.localPosition().z();

	  if (verbose) std::cout << "AR tsosX: " << tsosX << std::endl;
	  if (verbose) std::cout << "AR tsosY: " << tsosY << std::endl;
	  if (verbose) std::cout << "AR tsosZ: " << tsosZ << std::endl;
          ...
          if ( trajMeasurementHitDim > 1 ) {
            ...
            if (verbose) std::cout << "          vDTSeg2D size: " << vDTSeg2D.size() << std::endl;
            for ( std::vector<const TrackingRecHit*>::const_iterator itDTSeg2D =  vDTSeg2D.begin();
              ...
              if (verbose) std::cout << "            vDTHits1D size: " << vDTHits1D.size() << std::endl;
              for ( std::vector<const TrackingRecHit*>::const_iterator itDTHits1D =  vDTHits1D.begin();
                ...
                if (verbose) std::cout << "              hit dimension: " << hit->dimension() << std::endl;
                
                DetId hitId  = hit->geographicalId();
                const DTSuperLayerId superLayerId(hitId.rawId());
	              const DTLayerId layerId(hitId.rawId());
	              if (verbose) std::cout << "              hit superLayerId: " << superLayerId.superLayer() << std::endl;
	              if (verbose) std::cout << "              hit layerId: " << layerId.layer() << std::endl;
                ...
                if ( superLayerId.superlayer() == 2  && vDTHits1D.size() >= 3 ) {
                  if ( m_dt2.find(chamberId) == m_dt2.end() ) {
                    ...
                    m_dt2[chamberId] = new MuonDT2ChamberResidual(globalGeometry, navigator, chamberId, chamberAlignable);
	                  if (verbose) std::cout << "              This is first appearance of the DT with hits in superlayer 2" << std::endl;
	                  
                  }
		  if (verbose) std::cout << "This add residual not properly removed AR" << std::endl;
		  m_dt2[chamberId]->addResidual(prop, &tsos, hit);
                  ...
                  if ( m_dt13.find(chamberId) == m_dt13.end() ) {
                    ...
                    if (verbose) std::cout << "              This is first appearance of the DT with hits in superlayers 1 and 3" << std::endl;
            	    }
		  if (verbose) std::cout << "This add residual not properly removed AR" << std::endl;
          ...
          if ( residualDT13IsAdded ==true && residualDT2IsAdded == true && chamberId.wheel() == 0 && chamberId.station() == 2 && chamberId.sector() == 7 ) {
            if (verbose) std::cout << "MYMARK " << tsosX << " " << hitX << " " << tsosX - hitX << " " << m_dt13[chamberId]->trackx() << " " << m_dt13[chamberId]->residual()
...
// YP I add false here. No trajectory measurments in Muon system if we corrected TrackTransformer accordingly
          ...
          if (verbose) std::cout << "        TrajectoryMeasurement hit subDet: CSC endcap " << cscDetId.endcap() << " station " << cscDetId.station() << " ring " << cscDetId.ring() << " chamber " << cscDetId.chamber() << std::endl;
          if (verbose) std::cout << "        TrajectoryMeasurement hit dimension: " << trajMeasurementHitDim << std::endl;
          
          if ( trajMeasurementHitDim == 4 ) {
            std::vector<const TrackingRecHit*> vCSCHits2D = trajMeasurementHit->recHits();
            if (verbose) std::cout << "          vCSCHits2D size: " << vCSCHits2D.size() << std::endl;
            if ( vCSCHits2D.size() >= 5 ) {
              for ( std::vector<const TrackingRecHit*>::const_iterator itCSCHits2D =  vCSCHits2D.begin();
                ...
                if (verbose) std::cout << "            cscHit2D dimension: " << cscHit2D->dimension() << std::endl;
                const TrackingRecHit* hit = cscHit2D;
                if (verbose) std::cout << "              hit dimension: " << hit->dimension() << std::endl;
                
                DetId hitId  = hit->geographicalId();
                const CSCDetId cscDetId(hitId.rawId());
                if (verbose) std::cout << "              hit layer: " << cscDetId.layer() << std::endl;
                ...
                if ( m_csc.find(chamberId2) == m_csc.end() ) {
                  ...
                  m_csc[chamberId2] = new MuonCSCChamberResidual(globalGeometry, navigator, chamberId2, chamberAlignable);
	                if (verbose) std::cout << "              This is first appearance of the CSC with hits QQQ" << std::endl;
            	  }
          ...
          if (verbose) std::cout << "        TrajectoryMeasurement hit subDet: UNKNOWN" << std::endl;
        }
      } else {
        if (verbose) std::cout << "      TrajectoryMeasurement hit det: UNKNOWN" << std::endl;
      }
    }
  }    
  ...
  for (trackingRecHit_iterator hit2 = m_recoTrack->recHitsBegin(); hit2 != m_recoTrack->recHitsEnd(); ++hit2) {
    if((*hit2)->isValid()) {
      ...
      if ( hitId2.det() == DetId::Tracker ) {
        iT2++;
        if (verbose) std::cout << "Tracker Hit " << iT2 << " is found. We don't calcualte Tsos for it" << std::endl;
...
//        }
          iM2++;
          if (verbose) std::cout << "Muon Hit " << iM2 << " is found. Dimension: " << (*hit2)->dimension() << std::endl;
              ...
              for ( std::vector<const TrackingRecHit*>::const_iterator itDTHits1D =  vDTHits1D.begin();
                ...
                if ( superLayerId.superlayer() == 2  && vDTHits1D.size() >= 3 ) {
                  ...
                  if ( extrapolation.isValid() ) {
            	    if (verbose) std::cout << " extrapolation localPosition()"
            	              << " x: " << extrapolation.localPosition().x()
            	              << " y: " << extrapolation.localPosition().y() 
            	              << " z: " << extrapolation.localPosition().z() << std::endl;
		    //AR added as alternative method of doing fiducial cuts

		    m_dt2[chamberId]->addResidual(prop, &extrapolation, hit);

		    /*		    if (fabs(extrapolation.localPosition().x())<80 && fabs(extrapolation.localPosition().y()) < 80){
		      }
		    else{ if (verbose) std::cout << "AR residual removed, extrapolated.localPosition.x(): " << extrapolation.localPosition().x() << ", extrapolated.localPosition.y(): " << extrapolation.localPosition().y() << std::endl;}*/
                  }
//            	    residualDT2IsAdded = true;
...
//          if (verbose) std::cout << "Extrapolate last Tracker TSOS to muon hit" << std::endl;
//            TrajectoryStateOnSurface extrapolation;
...
//            
//            if (verbose) std::cout << "MYMARK " << tsosX2 << " " << hitX2 << " " << tsosX2 - hitX2 << " " << "0" << " " << "0"
...
//            }
            ...
            if (verbose) std::cout << "Muon hit in CSC endcap " << cscDetId2.endcap() << " station " << cscDetId2.station() << " ring " << cscDetId2.ring() << " chamber " << cscDetId2.chamber() << "." << std::endl;
            ...
            if ( vCSCHits2D.size() >= 5 ) {
              for ( std::vector<const TrackingRecHit*>::const_iterator itCSCHits2D =  vCSCHits2D.begin();
                ...
                if (verbose) std::cout << " hit localPosition"
            	            << " x: " << hit->localPosition().x()
            	            << " y: " << hit->localPosition().y() 
            	            << " z: " << hit->localPosition().z()
            	            << std::endl;
            	  if (verbose) std::cout << " hit globalPosition"
                ...
                // have we seen this chamber before?
                if (verbose) std::cout << "Have we seen this chamber before?";
                if ( m_csc.find(chamberId) == m_csc.end() ) {
                  if (verbose) std::cout << " NO. m_csc.count() = " << m_csc.count(chamberId) << std::endl;
                  AlignableDetOrUnitPtr chamberAlignable = navigator->alignableFromDetId(chamberId);
                  m_csc[chamberId] = new MuonCSCChamberResidual(globalGeometry, navigator, chamberId, chamberAlignable);
	                if (verbose) std::cout << "              This is first appearance of the CSC with hits m_csc.count() = " << m_csc.count(chamberId) << std::endl;
	                m_chamberIds.push_back(chamberId);
                  //addTrkCovMatrix(chamberId, tsos); // only for the 1st hit
            	  } else {
            	    if (verbose) std::cout << " YES. m_csc.count() = " << m_csc.count(chamberId) << std::endl;
            	  }
            	  
            	  if (verbose) std::cout << " lastTrackerTsos localPosition"
            	            << " x: " << lastTrackerTsos.localPosition().x()
            	            << " y: " << lastTrackerTsos.localPosition().y() 
            	            << " z: " << lastTrackerTsos.localPosition().z()
            	            << std::endl;
            	  if (verbose) std::cout << " lastTrackerTsos globalPosition"
            	            << " x: " << lastTrackerTsos.globalPosition().x()
            	            << " y: " << lastTrackerTsos.globalPosition().y() 
            	            << " z: " << lastTrackerTsos.globalPosition().z()
            	            << std::endl;
            	  if (verbose) std::cout << " Do extrapolation from lastTrackerTsos to hit surface" << std::endl;
            	  TrajectoryStateOnSurface extrapolation;
                extrapolation = prop->propagate( lastTrackerTsos, globalGeometry->idToDet(hitId)->surface() );
            	  if (verbose) std::cout << " extrapolation.isValid() = " << extrapolation.isValid() << std::endl;
...
//        recHitsForRefit.push_back(theMuonRecHitBuilder->build(&**hit));
  ...
  if (verbose) std::cout << "END MuonResidualsFromTrack" << std::endl << std::endl;
}
